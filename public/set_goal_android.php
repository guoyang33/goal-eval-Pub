 ...
 * 2023-10-23: baseline_usage_time_mean的sql查詢，week參數設為恆為0
 */
...
if ($goal['start_date'] != null && $user['test'] != 1) {
    ...
    <p>本週已經設定過目標囉</p>';
    echo '<button type="button" class="btn btn-secondary" onclick=location.href="redirect.php?u_id='.$user['id'].'">
    返回主頁面
    </button>
    </div>';
} else {
    ...
    if ($week == 1) {       // 第1週
        // 查詢category_usage_manual_android資料表中第0週的資料
        // 因為資料表中紀錄的usage_time是以分鐘為單位，所以要乘上60
        $sth = $dbh->prepare('SELECT category, usage_time * 60 AS usage_time_sec FROM `category_usage_manual_android` WHERE `u_id`=:u_id AND `week` = 0;');
        $sth->execute([
            'u_id' => $user['id']
        ]);
        foreach ($sth->fetchAll(PDO::FETCH_ASSOC) as $row) {
            if (in_array(strtoupper($row['category']), array_keys($baseline_usage_time_mean))) {
                if (strtoupper($row['category']) === 'COMMUNICATION') {
                    // 因為communication類別只收工作日(5天)的時間，所以除以5
                    $usage_time_mean = $row['usage_time_sec'] / 5;
                    $baseline_usage_time_mean[strtoupper($row['category'])] = $usage_time_mean;
                    $last_week_usage_time_mean[strtoupper($row['category'])] = $usage_time_mean;
                } else {
                    $usage_time_mean = $row['usage_time_sec'] / 7;
                    $baseline_usage_time_mean[strtoupper($row['category'])] = $usage_time_mean;
                    $last_week_usage_time_mean[strtoupper($row['category'])] = $usage_time_mean;
                }
            }
        ...
        // 查詢category_usage_manual_android資料表中第0週乘上有達標週減量目標的資料
        // 因為資料表中紀錄的usage_time是以分鐘為單位，所以要乘上60
        foreach (array_keys($baseline_usage_time_mean) as $baseline_category) {
            ...
            if ($baseline_category === 'COMMUNICATION') {      // 因為communication類別只收工作日(5天)的時間，所以除以5
                $baseline_usage_time_mean[$baseline_category] = ($sth->fetchAll(PDO::FETCH_ASSOC))[0]['usage_time'] * 60 / 5;
            } else {
        ...
        foreach ($last_week_reach_goal as $is_reach_goal_category => $is_reach_goal) {
            if (!boolval($is_reach_goal)) {
                ...
                if ($need_save_category === 'communication') {
                    $all_need_save_time[$need_save_category] = ($sth->fetchAll(PDO::FETCH_ASSOC))[0]["score_save_{$need_save_category}_time"] * 60 / 5;
                } else {
        ...
        // 查詢category_usage_manual_android資料表中第N-1週的資料
        // 因為資料表中紀錄的usage_time是以分鐘為單位，所以要乘上60
        $sth = $dbh->prepare('SELECT category, usage_time * 60 AS usage_time_sec FROM `category_usage_manual_android` WHERE `u_id`=:u_id AND `week`=:last_week;');
        $sth->execute([
            'u_id' => $user['id'],
            'last_week' => $week - 1
        ]);
    ...
    <form action="set_goal_android_submit.php" method="POST">
    ...
    foreach (MAIN_APP_CATEGORY as $app_category => $category_chinese) {
        if ($baseline_usage_time_mean[$app_category] == 0) {
            ...
            for ($i=3; $i<=10; $i++) {
                $reduce_rate = 1 - ($i/100);
                // if ($app_category === 'COMMUNICATION') {
                //     $usage_time_reduce_formatted = time_beautifier(ceil($new_baseline_usage_time * 60 / 5 * $reduce_rate));
                // } else {
