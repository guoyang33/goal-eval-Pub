...
if ($user['exp_id'][0] == 'I') {
    header("Location: app_usage_lookup_and_rank_ios.php?u_id={$user['id']}&app_category={$category}");
    exit();
}
...
echo '<br><button type="button" class="btn btn-primary" onclick="window.location.replace(' . "'redirect.php?u_id={$user['id']}'" . ');">
返回主選單
</button>';
