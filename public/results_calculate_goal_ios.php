...
$week = $week - 1;      // 因為是下周一結算上週全部的所以week要-1
...
// 計算補救分數(第二週之後才算)
if ($week > 1) {
    Log::d("update_category_save_sql_string_array: " . var_export($update_category_save_sql_string_array, true));
...
if ($updateScoreSuccess === false) {
    ...
    echo '<div class="d-grid pt-5 pb-3 px-2">
    <h2>上週使用時間結算完成</h2><br>';
    $sth = $dbh->prepare('SELECT ' . implode(',', $is_reach_goal_sql_string_array) . ' FROM `goal` WHERE `u_id`=:u_id AND `week`=:week LIMIT 1;');
    $sth->execute([
        'u_id' => $user['id'],
        'week' => $week
    ]);
