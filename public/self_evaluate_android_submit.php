...
if ($form_answer === false) {
    ...
    // 寫入Android手動填入使用時間的資料表
    $sql_value_string_array = array();
    $sql_value_array = array();
    foreach ($_POST['answer_sheet']['self_evaluate'][$user['exp_type']]['usage_time'] as $category => $avg_usage_time) {
        if (strpos($category, 'mobile_') === 0) {
			if (!in_array('holiday', array_keys($avg_usage_time))) {
				$avg_usage_time['holiday'] = 0;
			}
            array_push($sql_value_string_array, '(NULL,?,?,?,?,?,NULL)');
            array_push($sql_value_array, $user['id'], str_replace('mobile_', '', $category), (($avg_usage_time['workday'] * 5) + ($avg_usage_time['holiday'] * 2)), $today->format('Y-m-d'), $week);
        }
    }
    $sth = $dbh->prepare("INSERT INTO `category_usage_manual_android` (`id`, `u_id`, `category`, `usage_time`, `submit_date`, `week`, `makeup`) VALUE " . implode(',', $sql_value_string_array));
