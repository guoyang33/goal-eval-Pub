...
$reduce_entertainment = $reduce_rate['ENTERTAINMENT'];
...
if ($goal === false) {      // 插入資料
    $sth = $dbh->prepare("INSERT INTO `goal` (`u_id`, `week`, `start_date`, `reduce_game`, `reduce_social`, `reduce_entertainment`, `reduce_total`) VALUE (:u_id, :week, :start_date, :reduce_game, :reduce_social, :reduce_entertainment, :reduce_total);");
    $sth->execute([
        'u_id' =>                   $user['id'],
        'week' =>                   $week,
    ...
    $sth = $dbh->prepare("UPDATE `goal` SET `start_date`=:start_date, `reduce_game`=:reduce_game, `reduce_social`=:reduce_social, `reduce_entertainment`=:reduce_entertainment, `reduce_total`=:reduce_total WHERE id=:g_id");
    $sth->execute([
        'start_date' =>             ($start_date->modify((($week * 7) + 1) . ' day'))->format('Y-m-d'),
        'reduce_game' =>            $reduce_rate['GAME'],
        'reduce_social' =>          $reduce_rate['SOCIAL'],
...
// 新增進度(有效天數)
$insert_valid_days_status = insert_provide_valid_days($user['id'], PROVIDE_VALID_DAYS[$user['exp_type']]['set_goal_ios'], basename(__FILE__), 'set_goal_ios', $week);
