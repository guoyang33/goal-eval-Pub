...
$MAIN_APP_CATEGORY = array(
    ...
    'ENTERTAINMENT' => array(
        'zh' => '娛樂',
        'reduce' => 'reduce_entertainment',
        'score' => 'reduce_entertainment',
        'sql_score_update' => "UPDATE `goal` SET reduce_entertainment=:reduce_entertainment WHERE id=:goal_id",
        'makeup' => 'reduce_entertainment'
    )
);
...
if ($week < 0) {    // Week -1
    ...
    $formAnswerSelfEvaluateIOS = get_form_answer_self_evaluate_ios($user['id'], $user['exp_type'], ($week - 1));
    Log::d('form answer self evaluate ios: ' . var_export($formAnswerSelfEvaluateIOS, true));

    if ($formAnswerSelfEvaluateIOS === false) {      // 如果上週評估單尚未填寫
		include 'self_evaluate_ios.php';
	} else {
        if ($week > 1) {      // 除第一週不需計算分數，之後都需要
            try {
                $resultsCalculateGoal = get_results_calculate_goal(OBJECT_DEVICE, $user['id'], ($week - 1));
                Log::d('result calculate goal: ', var_export($resultsCalculateGoal, true));
            ...
            if (!$resultsCalculateGoal) {      // 檢查是否已結算上週分數
                include 'results_calculate_goal_ios.php';      // 計算分數
                die();
            }
        }
...
// 策略訓練方案-實踐引言
        ...
        // 查看擁有的獎勵拼圖
        echo '<br><button type="button" class="btn btn-primary" onclick=location.href="goal_ios.php?u_id=' . $user['id'] . '">
        查看擁有的獎勵拼圖
        </button>';

        // 查看APP使用時間
        foreach ($MAIN_APP_CATEGORY as $app_category => $category) {
            echo '<button type="button" class="btn btn-primary mt-3" onclick=location.href="app_usage_lookup_and_rank_ios.php?u_id='.$user['id'].'&app_category='.$app_category.'">查看'.$category['zh'].'APP使用時間</button>';
        }
        echo '<!-- 查看APP總使用時間 -->
        <button type="button" class="btn btn-primary mt-3" onclick=location.href="app_usage_lookup_and_rank_ios.php?u_id='.$user['id'].'&app_category=TOTAL">查看APP總使用時間</button>
        </div>';
    }
} else if ($week >= 9 && $week <= 24) {      // 第9-24週
    if (get_form_answer_self_evaluate_ios($user['id'], $user['exp_type'], ($week - 1)) === false) {      // 如果上週評估單尚未填寫
		include 'self_evaluate_ios.php';
	} else {
