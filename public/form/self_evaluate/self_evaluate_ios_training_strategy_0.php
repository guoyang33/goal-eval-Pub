...
?>
        ...
        <div class="d-grid pt-5 pb-3 px-2 text-center">
            <h2>請依照您在預備週時<strong>手機</strong>及<strong>電腦</strong>使用的的情況回答下列問題，幫助我們暸解您上週<strong class="text-info"><?php echo '(' . (($start_date->modify((1 + $week * 7) . ' day'))->format('Y-m-d')) . '~' . (($start_date->modify((($week + 1) * 7) . ' day'))->format('Y-m-d')) . ')'; ?></strong>的情形，同時也請您回顧和思考。（手機使用時間請參照iPhone的「螢幕使用時間」功能，電腦使用時間請自行評估）</h2>
        </div>
